...
if menu == "📊 Dashboard & Cumplimiento":
    ...
    with st.form("f_venta"):
        ...
        if st.form_submit_button("Registrar"):
            if not cliente:
            ...
            elif lleva_t == "Sí" and cant_t > stock_act:
                st.error("Stock insuficiente.")
            else:
                if lleva_t == "Sí" and tarj_sel:
                ...
                creditos.append({
                    ...
                    "Tarjeta Entregada": tarj_sel if lleva_t == "Sí" else "N/A",
                    "Cantidad": 1,
                    "Asesor": asesor,
                })
    ...
    pendientes = [(i, c) for i, c in enumerate(creditos) if c.get("¿Lleva Tarjeta?") == "No"]

    if not pendientes:
        st.info("No hay créditos pendientes de tarjeta.")
    elif not inventario:
        ...
        with st.form("f_pend"):
            ...
            for i, c in pendientes:
                ops.append(f"#{i} - {c.get('Cliente')} - {c.get('Marca de Celular')}")
    ...
    if activar_f:
        c1, c2 = st.columns(2)
        with c1: f_ini = st.date_input("Inicio", obtener_hora().date())
        with c2: f_fin = st.date_input("Fin", obtener_hora().date())
    ...
    if sub == "Créditos":
        ...
        if not es_admin:
            ...
            if st.button("🗑️ Anular"):
                ...
                if item.get("¿Lleva Tarjeta?") == "Sí" and item.get("Tarjeta Entregada") != "N/A":
